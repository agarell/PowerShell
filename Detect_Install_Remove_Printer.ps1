...
if ($mappedPrinters.Name -notcontains "\\printserver\Follow-Me-ARM" -and (Test-Connection print.server.com -Count 1 -Quiet)) {
    Exit 1
} 
else {
    Exit 0
...
Add-Printer -ConnectionName "\\printserver\Follow-Me-ARM" 
...
New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Windows' -Name 'Device' -Value '\\printserver\Follow-Me-ARM,winspool,Ne02:' -PropertyType String -Force -ea SilentlyContinue;
...
foreach ($printer in $removePrinters) {
    if ($printer.Name -like "\\printserver\*" -and $printer.Name -ne '\\printerserver\Follow-Me-ARM') {
